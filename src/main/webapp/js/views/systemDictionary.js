$(function() {
	var systemDictionary_datagrid, systemDictionaryItem_datagrid,
		systemDictionaryItem_dialog, systemDictionaryItem_form, systemDictionaryItem_tb;
	...
	systemDictionaryItem_tb = $("#systemDictionaryItem_tb");
	...
	systemDictionaryItem_datagrid.datagrid({
		...
		onClickRow: function(rowIndex, rowData) {
			...
			if(rowData.state) {
				systemDictionaryItem_tb.eq(1).linkbutton("enable");
				systemDictionaryItem_tb.eq(2).linkbutton("enable");
			} else {
				systemDictionaryItem_tb.eq(1).linkbutton("disable");
				systemDictionaryItem_tb.eq(2).linkbutton("disable");
			}
		},
		columns: [[
	...
	var cmdObj = {
		...
		add: function() {
			...
			if(rowData != null) {
				...
				$.messager.alert("温馨提示", "请选中要添加的字典项目", "info");
			}
